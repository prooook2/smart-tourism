  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const hasTickets = res.data.event.ticketTypes?.length > 0;
        setUseTicketTypes(hasTickets);
        setTicketTypes(
          hasTickets
            ? res.data.event.ticketTypes.map((t) => ({
                _id: t._id,
                label: t.label,
          ...
          {useTicketTypes && (
            <div className="space-y-4 rounded-2xl border border-pink-100 bg-white p-4">
              ...
              {ticketTypes.map((t, idx) => (
                <div key={t._id || idx} className="rounded-xl border border-dusk/10 bg-secondary p-4 space-y-3">
